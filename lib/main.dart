...
class _MyHomePageState extends State<MyHomePage> {
  ...
  // 将物品按分类分组（支持二级分类）
  Map<String, Map<String, List<TodoItem>>>
  _groupItemsByCategoryWithSubcategories() {
    final grouped = <String, Map<String, List<TodoItem>>>{};

    for (var item in _items) {
      // 获取主分类名称
      final mainCategory = ClothingCategories.getMainCategoryName(
        item.category,
      );
      ...
      if (!grouped.containsKey(mainCategory)) {
        ...
        _expandedCategories.putIfAbsent(mainCategory, () => false);
      }

      // 获取子分类名称（如果没有子分类则使用主分类）
      final subCategory = item.subcategory ?? item.category;
      ...
      grouped[mainCategory]![subCategory]!.add(item);
    }
  ...
  // 构建侧边栏目录（支持二级分类）
  Widget _buildDrawer() {
    final groupedItems = _groupItemsByCategoryWithSubcategories();
    final sortedMainCategories = groupedItems.keys.toList()..sort();
        ...
        children: [
          ...
          Expanded(
            child: groupedItems.isEmpty
                ...
                : ListView.builder(
                    itemCount: sortedMainCategories.length,
                    itemBuilder: (context, index) {
                      final mainCategory = sortedMainCategories[index];
                      final subCategories = groupedItems[mainCategory]!;
                      final isExpanded =
                          _expandedCategories[mainCategory] ?? false;
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => AllItemsPage(
                  items: _items,
                  onDeleteItem: _deleteItem, // 传递删除回调
                ),
              ),
            );
          },
  ...
  Widget _buildMainCategoryItem(
    String mainCategory,
    Map<String, List<TodoItem>> subCategories,
    bool isExpanded,
  ) {
    // 为不同主分类设置不同颜色
    final categoryColors = [
    ...
    final categoryIndex = mainCategory.hashCode % categoryColors.length;
    final categoryColor = categoryColors[categoryIndex];
      ...
      children: [
        ...
        ListTile(
          leading: CircleAvatar(
            ...
            child: Icon(Icons.folder, size: 18, color: categoryColor),
          ),
          title: Text(
            mainCategory,
          ...
          trailing: Row(
            ...
            children: [
              Container(
                ...
                child: Text(
                  '${subCategories.values.fold(0, (sum, items) => sum + items.length)}',
                  style: TextStyle(
                    color: categoryColor,
                    fontSize: 12,
                    fontWeight: FontWeight.bold,
              ...
              IconButton(
                ...
                onPressed: () {
                  setState(() {
                    _expandedCategories[mainCategory] = !isExpanded;
                  });
                },
              ),
            ],
          ...
          Container(
            ...
            child: Column(
              children: subCategories.entries.map((entry) {
                ...
                final isSubExpanded = _expandedCategories[subCategory] ?? false;
      ...
      children: [
        ListTile(
          ...
          leading: Icon(
            ...
            color: mainCategoryColor.withOpacity(0.7),
            size: 20,
          ),
          title: Text(
            subCategory,
            style: TextStyle(
              fontSize: 14,
              color: mainCategoryColor.withOpacity(0.9),
            ),
          ),
          trailing: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: mainCategoryColor.withOpacity(0.1),
              ...
              IconButton(
                ...
                onPressed: () {
                  setState(() {
                    _expandedCategories[subCategory] = !isExpanded;
                  });
                },
              ),
            ],
